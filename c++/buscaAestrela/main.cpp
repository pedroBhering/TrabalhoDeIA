        ...
        for (size_t k = 0; k < no->actions.size(); ++k)
        {
            ...
            if (explored.count(matriz_aux) == 0)
            {
                ...
                if (no->i < n - 1)
                {
                    ...
                    peso_aresta = qtdMatrizNivel[no->j * n + (no->i + 1) - n];// numero de vezes que a posição foi acessada
                    peso_caminho = (no->parent != NULL) ? no->parent->w_caminho + peso_aresta : 0; // peso do caminho + atual
                    peso_ae = peso_aresta + peso_caminho;
